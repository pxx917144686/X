...
void jbupdate_update_system_info(void)
{
	@autoreleasepool {
		...
		if (!xpfHandle) {
			char msg[4000];
			snprintf(msg, 4000, "X: dlopening libxpf failed: (%s), cannot continue.", dlerror());
		...
		if (error) {
			char msg[4000];
			snprintf(msg, 4000, "X: Updating system info via XPF failed with error: (%s), cannot continue.", error);
			abort_with_reason(7, 1, msg, 0);
			return;
		}
...
void jbupdate_finalize_stage2(const char *prevVersion, const char *newVersion)
{
	...
	if (r != 0) {
		char msg[4000];
		snprintf(msg, 4000, "X: Updating patched dyld failed with error %d, cannot continue.", r);
		abort_with_reason(7, 1, msg, 0);
	}
	...
	if (cdhashesCount > 1) {
		char msg[4000];
		snprintf(msg, 4000, "X: Updating patched dyld failed due to unexpected amount of cdhashes (%d), cannot continue.", cdhashesCount);
		abort_with_reason(7, 1, msg, 0);
	}
		...
		if (r != 0) {
			char msg[4000];
			snprintf(msg, 4000, "X: Building dyld trustcache failed with error %d, cannot continue.", r);
			abort_with_reason(7, 1, msg, 0);
		}
		...
		if (r != 0) {
			char msg[4000];
			snprintf(msg, 4000, "X: Updating dyld trustcache failed with error %d, cannot continue.", r);
			abort_with_reason(7, 1, msg, 0);
		}
