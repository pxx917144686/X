...
int basebin_generate(bool comingFromJBUpdate)
{
	...
	NSString *xVersion = [NSString stringWithContentsOfFile:JBROOT_PATH(@"/basebin/.version") encoding:NSUTF8StringEncoding error:nil];
	if (!xVersion) {
	    // 创建X项目版本文件
	    NSString *defaultVersion = @"1.0.0";
	    [defaultVersion writeToFile:JBROOT_PATH(@"/basebin/.version") atomically:YES encoding:NSUTF8StringEncoding error:nil];
	    xVersion = defaultVersion;
	}
	...
	NSString *dyldUUIDPrefix = [@"DOPA" stringByAppendingString:xVersion];
